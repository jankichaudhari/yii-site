...
$sql = "SELECT * FROM cstatus WHERE cst_scope = '".$_SESSION["auth"]["default_scope"]."'";
$q = $db->query($sql);
while ($row = $q->fetchRow()) {
...
// lettings sapecific forms
else {

	$price_brackets = array(
		'0-250'=>'&pound;0 to &pound;250',
		'250-350'=>'&pound;250 to &pound;350',
		'350-500'=>'&pound;350 to &pound;500',
		'500-750'=>'&pound;500 to &pound;750',
		'750-1000'=>'&pound;750 to &pound;1000',
		'1000-2000'=>'&pound;1000 to &pound;2000'
		);
	$deposits = array (
		'0-15'=>'0-15%',
		'16-25'=>'16-25%',
	...
	$form3 = array(
		'cli_workstatus'=>array(
			'type'=>'select',
			'label'=>'Work Status',
			'required'=>2,
			'options'=>db_enum("client","cli_workstatus","array"),
			'value'=>$cli_workstatus
			),
		'cli_moveby'=>array(
			'type'=>'text',
			'label'=>'Move by date? (YYYY-MM-DD)',
			'required'=>2,
			'value'=>$cli_moveby
			),
		'cli_student'=>array(
...
$sql = "SELECT * FROM branch WHERE bra_title LIKE '%".$_SESSION["auth"]["default_scope"]."%'";
$q = $db->query($sql);
while ($row = $q->fetchRow()) {
...
$page->display();
	...
	if ($_SESSION["auth"]["default_scope"] == 'Sales') {
		...
		$db_data["cli_letstatus"] = $_POST["cli_saleletstatus"];
		$db_data["cli_letbed"] = $_POST["cli_letbed"];
		$db_data["cli_letemail"] = $_POST["cli_letemail"];
		$db_data["cli_lettings"] = 'Yes';
		$db_data["cli_letptype"] = remove_lastchar($cli_ptype,'|');
		$db_data["cli_letmin"] = $minprice;
		$db_data["cli_letmax"] = $maxprice;
		}
