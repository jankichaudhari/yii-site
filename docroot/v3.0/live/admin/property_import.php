...
/*
property_import
for adding property to the system during import period. allows dates to be manually entered
*/

// default vendor to me for today, or bruce if gemma is using the system to add lettings
if ($_SESSION["auth"]["use_id"] == 1) {
	$_GET["cli_id"] = 1;
	$status = 'Under Offer';

	} elseif ($_SESSION["auth"]["use_id"] == 9) {
	$status = 'Available';
	$_GET["cli_id"] = 2;
	$_SESSION["auth"]["default_scope"] = 'Lettings';

	} elseif ($_SESSION["auth"]["use_id"] == 3) {
	$status = 'Exchanged';
	$_GET["cli_id"] = 1;
	//$_SESSION["auth"]["default_scope"] = 'Lettings';

	} elseif ($_SESSION["auth"]["use_id"] == 48) {
	$status = 'Under Offer';
	$_GET["cli_id"] = 2;
	$_SESSION["auth"]["default_scope"] = 'Lettings';
	}

	elseif ($_SESSION["auth"]["use_id"] == 41) {
	$status = 'Under Offer';
	$_GET["cli_id"] = 2;
	$_SESSION["auth"]["default_scope"] = 'Lettings';
	}

	elseif ($_SESSION["auth"]["use_id"] == 16) {
	$status = 'Under Offer';
	$_GET["cli_id"] = 2;
	$_SESSION["auth"]["default_scope"] = 'Lettings';
	}
...
$form->addHtml('<div class="block-header">Property Address</div>');
...
$sql = "SELECT deal.dea_type,deal.dea_ptype,dea_psubtype,dea_floors,dea_floor,dea_reception,dea_bedroom,dea_bathroom,
...
var popcaldea_date = new CalendarPopup("popCalDivdea_date");
');
...
// update sot with supplied date
if ($dea_date) {

	$date_parts = explode("/",$dea_date);
	$dea_date = $date_parts[2].'-'.$date_parts[1].'-'.$date_parts[0].' 00:00:00';

	$sql = "SELECT sot_id FROM sot WHERE sot_deal = $dea_id";
	$q = $db->query($sql);
	while ($row = $q->fetchRow()) {
		$sot_id = $row["sot_id"];
		}
	$db_data_sot["sot_date"] = $dea_date;
	$sot_id = db_query($db_data_sot,"UPDATE","sot","sot_id",$sot_id);
	}
...
if (!$_GET["app_id"]) {
	...
	$sql = "SELECT cli_id FROM link_client_to_instruction
	LEFT JOIN client ON link_client_to_instruction.clientId = client.cli_id
	WHERE
	link_client_to_instruction.dealId = $dea_id AND link_client_to_instruction.clientId != $cli_id";
	$q = $db->query($sql);
	if (DB::isError($q)) {  die("db error: ".$q->getMessage()); }
	$numRows = $q->numRows();
	while ($row = $q->fetchRow()) {
