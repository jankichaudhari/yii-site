...
$formData1 = array(
	...
	'cli_method'=>array(
		...
		'options'=>array(''=>'','Website'=>'Self-registration on our site','Telephone'=>'Telephone','Email'=>'Email','Internet'=>'Internet (portal)','Walk-in'=>'Walk-in'),
...
DATE_FORMAT(cli_created, '%D %M %Y') AS date,tel.tel_number
FROM `client`
...
LEFT JOIN link_client_to_feature ON `client`.cli_id = link_client_to_feature.clientId AND status != 'Must not have'
...
foreach ($results as $key=>$val) {
	...
	foreach ($val as $subarray=>$values) {
		...
		<td onmousedown="document.getElementById(\'check_client_'.$values["cli_id"].'\').checked = (document.getElementById(\'check_client_'.$values["cli_id"].'\').checked ? false : true);">'.$values["cli_tel"].'</td>
