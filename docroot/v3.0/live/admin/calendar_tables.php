...
*/
$render = '<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr>
<td valign="top">
  <div id="calFilter">
	<table width="100%" cellpadding="3" cellspacing="0">
	  <tr>
		<td class="calTitleDate"><a href="'.$calendar_url.'" target="cal_iframe" title="Click to refresh calendar">'.date('l, jS of F y',$Day->getTimeStamp()).'</a></td>
		<!--<td><a href="calendar.php?'.$query_string.'do=zoom&zoom='.$next_zoom.'">Zoom</a></td>-->
		<!--<td align="right" width="20">Branch:</td>-->
		<td align="left" width="140">

<form method="GET" name="branch">
<select name="branch" onChange="document.branch.submit()" style="width:140px">
'.$render_branch.'
</select>';
foreach ($to_pass AS $key=>$val) {
if ($key !== 'branch') {
...
<form method="GET" name="type">
<select name="type" onChange="document.type.submit()" style="width:85px">
'.$render_type.'
</select>
';
foreach ($to_pass AS $key=>$val) {
...
<select name="user" onChange="document.user.submit()" style="width:125px">
'.$render_user.'
</select>';
foreach ($to_pass AS $key=>$val) {
if ($key !== 'user') {
