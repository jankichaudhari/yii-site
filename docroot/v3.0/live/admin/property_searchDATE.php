...
$formData1 = array(
	...
	'status_special'=>array(
		'type'=>'radio',
		'label'=>'Status<br><br><br><br><br><br><br><br>',
...
if ($_GET["cli_id"]) {
	...
	if (DB::isError($q_cli)) {  die("db error: ".$q->getMessage().$sql_cli); }
...
#LEFT JOIN client ON deal.dea_vendor = client.cli_id
...
if ($numRows !== 0) {
	while ($row = $q->fetchRow()) {
		...
		$data[] = '
		<tr class="trOff" onMouseOver="trOver(this)" onMouseOut="trOut(this)" ondblclick="document.location.href = \'deal_summary.php?dea_id='.$row["dea_id"].'&searchLink='.$searchLink.'\'">
		...
		<td width="100" onmousedown="document.getElementById(\'check_deal_'.$row["dea_id"].'\').checked = (document.getElementById(\'check_deal_'.$row["dea_id"].'\').checked ? false : true);">'.$row["dateUpdated"].'</td>
...
<table>
  <tr>
	...
	array('title'=>'Date','column'=>'dateUpdated'),
