...
# dbtree, this inc also include the CDatabase class to work with dbtree.
...
// my functions, should be incorporated into teh vlass at some stage

function display_tree($root) { 
	// retrieve the left and right value of the $root node 
	$result = mysql_query('SELECT cat_left, cat_right FROM category '. 
	'WHERE cat_id="'.$root.'";'); 
	...
	$sql = 'SELECT cat_id,cat_title, cat_left, cat_right, cat_level, cat_title  
	FROM category 
	WHERE cat_left BETWEEN '.$row['cat_left'].' AND '.$row['cat_right'].' 
	...
	while ($row = mysql_fetch_array($result)) { 
		...
		if (count($right)>0) { 
			...
			while ($right[count($right)-1]<$row['cat_right']) { 
				array_pop($right); 
				} 
			} 
			...
			if ($row["cat_id"] <> 1) {
			$output .= str_repeat('&nbsp;&nbsp;',count($right));			
			$output .= $row['cat_title'];
			
			if ($row["cat_level"] <> 1) {
				$output .= ' (<a href="?action=delete_node&node_id='.$row['cat_id'].'">x</a>)';
				}
			$output .= '<br>';
			}
...
function display_tree_select($root,$current_node=NULL,$style="select") { 
	// retrieve the left and right value of the $root node 
	$result = mysql_query('SELECT cat_left, cat_right FROM category '. 
	'WHERE cat_id="'.$root.'";'); 
	$row = mysql_fetch_array($result); 
	...
	while ($row = mysql_fetch_array($result)) { 
		...
		if (count($right)>0) { 
			...
			// display indented node title in select list
			if ($row["cat_id"] <> 1) {
			
			if ($style == "select") {
				$output .= '<option value="'.$row["cat_id"].'"';			
				if ($current_node == $row["cat_id"]) {
					$output .= ' selected';
					}
				$output .= '>'.str_repeat('&nbsp;&nbsp;',($row["cat_level"]-1)).$row['cat_title'].'</option>';
				}
			elseif ($style == "data") {
				$output[$row["cat_id"]] = str_repeat('&nbsp;&nbsp;',($row["cat_level"]-1)).$row['cat_title'];
				}
