...
$strPath = dirname(__FILE__) . '/zoomf/' . $strFolderName;
...
while ($row = $q->fetchRow()) {
	...
	if ($row["dea_status"] == STATUS_AVAILABLE) { //STATUS_ID - 0 for sale, 1 under offer
		$render .= "0" . "^";
	} elseif ($row["dea_status"] == STATUS_UNDER_OFFER || $row["dea_status"] == STATUS_UNDER_OFFER_WITH_OTHER) {
		$render .= "1" . "^";
	}
...
try {
	$ftp->upload($strPath, $CONFIG[FEED_NAME]['ftp_server']);
} catch (Exception $e) {
