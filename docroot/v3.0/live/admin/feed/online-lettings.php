<?php
define("FEED_NAME", "online-lettings");
require_once(dirname(__FILE__) . "/../../../../../config/config_feed.inc.php");
$strFolderName = date('Ymd');
$strPath       = dirname(__FILE__) . '/online-lettings/' . $strFolderName;
// name of textfile (date.blm)
$strTextFile = 'woosterstock.blm'; //date('Ymd').".blm";
$strZipFile  = date('Ymd') . ".zip";
...
$render .= "PUBLISHED_FLAG^LET_DATE_AVAILABLE^LET_BOND^LET_TYPE_ID^LET_FURN_ID^LET_RENT_FREQUENCY^TENURE_TYPE_ID^TRANS_TYPE_ID^NEW_HOME_FLAG^";
...
//$render .= "MEDIA_IMAGE_50^MEDIA_IMAGE_TEXT_50^";
...
// lettings branch codes
	if ($row["dea_branch"] == 1) {
		$intRMBranchID = 1001110;
	} elseif ($row["dea_branch"] == 2) {
		$intRMBranchID = 1001109;
	} elseif ($row["dea_branch"] == 3) {
		$intRMBranchID = 40674;
	} elseif ($row["dea_branch"] == 4) {
		$intRMBranchID = 1001108;
	}
	...
	$render .= "^"; //ADDRESS_3
	$render .= $row["are_title"] . "^"; //ADDRESS_4
	$render .= "London^"; //TOWN
...
// ensureing there are at least 3 features
	if (count($features) < 3) {
		$features[] = $row["ptype"];
		$features[] = $row["psubtype"];
		$features[] = $row["are_title"];
	}
	...
	$render .= round(intval($row["dea_marketprice"])) . "^"; //PRICE
...
// 5 floorplans
	...
	$render .= "^"; //MEDIA_IMAGE_50
	$render .= "^"; //MEDIA_IMAGE_TEXT_50
...
$render = str_replace("1.4E+6", "1400000", $render);
...
//$ftp_server   = "ftp.online-lettings.co.uk";
//$ftp_username = "wooster@online-lettings.co.uk";
//$ftp_password = "wstock";
