...
if ($_GET['branch']) {
	...
	if (DB::isError($result_colour)) {
		die("db error: " . $result_colour->getMessage() . $sql_colour);
	}
...
if ($numRows !== 0) {
	while ($row = $q->fetchRow()) {
		...
		$divs[] = array(
				'id'          => $row['followUpAppointmentId'] ? : $row['app_id'],
				...
				'end_pixel'    => $start_pixel + ($duration * $zoom),
				// multiply by $zoom becuase each minute = $zoom pixels
				'app_height'   => ($duration * $zoom),
				'colour'      => $row['use_colour'],
				'type'        => $row['app_type'],
				'subtype'     => $row['ity_title'],
				'notetype'    => $row['app_notetype'],
				'client'       => $cli_name,
			...
			if (($div['start_stamp'] >= $interval) && ($div['start_stamp'] < ($interval + $default_calendar_interval)) && in_array($div_id, $active)) {
				...
				switch ($div['type']):
					case "Viewing":
						...
						$caption = $div['type'] . ' ' . $div['start'] . ' to ' . $div['end'];
						$overdiv = "Client:\n" . str_replace("<br>", "\n", $div['client']) . ' - ' . $div['tel'] . "\n\nProperty:\n" . $div['addr'] . "\n\nNegotiator:\n" . $div['user'];
						if ($div['notes']) {
							$overdiv .= "\n\nNotes:\n" . $div['notes'];
						}
					...
					case "Valuation Follow Up":
						$title   = '<strong>' . $div['type'] . '</strong> ' . $div['start'] . '<br />' . $div['user'] . "<br>" . $div['subject'];
						$caption = $div['type'] . ' ' . $div['start'] . ' to ' . $div['end'];
						$overdiv = $div['subject'];
...
if (!$jumpDown && ($_GET['d'] == date('d') && $_GET['m'] == date('m') && $_GET['y'] == date('Y'))) {
...
$onLoad .= '
	...
	//document.getElementById("content_wide").style.display = \'block\';
	window.scroll(' . $jumpRight . ',' . $jumpDown . ');
	';
//window.focus();
...
// reload calendar after n seconds of mouse inactivity
var lastEvent = new Date().getTime();
var threshold = ' . $calendar_reload_delay . ';
function doReload(){
