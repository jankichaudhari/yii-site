...
/////////////////////////////////////////////////////////////////////////////
	case "client_address":
		if (isset($_GET['dea_id']) && !empty($_GET['dea_id'])) {
			header("Location:?dest=viewing&cli_id=" . $cli_id . "&dea_id=" . $_GET['dea_id'] . "&stage=viewing_address&skip=skip");
			exit;
		}
		header("Location:?dest=viewing&cli_id=" . $cli_id . "&stage=viewing_address&skip=skip");
		exit;
// see if we have client's old address on file and display
		...
		if (!$cli_oldaddr) {
			$sql         = "SELECT addressID FROM client WHERE cli_id = $cli_id";
			$cli_oldaddr = $db->getOne($sql);
		}
		if ($cli_oldaddr) {
			header("Location:?dest=viewing&cli_id=" . $cli_id . "&stage=viewing_address&skip=skip");
...
//			$form->addLegend('Client\'s Address');
			...
			$form->addHtml($form->addDiv($form->makeField("button", "", "", "Skip if Address Unknown", array(
					'onClick' => 'document.location.href = \'?' . $_SERVER['QUERY_STRING'] . '&stage=viewing_address&skip=skip\'',
					'class'   => 'submit'
			))));
...
//			header("Location:?stage=appointment&cli_id=" . $_GET["cli_id"] . "&dea_id=" . $carry_deal);
			...
			header("Location:?stage=appointment&cli_id=" . $_GET["cli_id"] . $app . $skip . "&dea_id=" . $carry_deal);
			exit;
		}
...
,DIY
,address.searchString AS pro_addr
FROM deal
...
LEFT JOIN address ON property.addressId = address.id
WHERE
$sql
...
ORDER BY $orderby $direction";
			...
			if ($numRows !== 0) {
				while ($row = $q->fetchRow()) {
					...
					$data[]   = '
		<tr class="trOff ' . ($row['DIY'] !== 'None' ? "{$row['DIY']}-property" : "") . '" onMouseOver="trOver(this)" onMouseOut="trOut(this)">
...
// appointment
	case "appointment":
		include __DIR__ . "/viewing_add_appointment.php";
		break;
// appointment
	case "requirements":
