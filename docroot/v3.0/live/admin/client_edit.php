...
GROUP_CONCAT(DISTINCT CONCAT(area.are_id) ORDER BY are_title ASC SEPARATOR '|') AS areas,
source.sou_title,
...
LEFT JOIN are2cli ON client.cli_id = are2cli.a2c_cli
LEFT JOIN area ON are2cli.a2c_are = area.are_id
...
if ($numRows == 0) {
	...
	while ($row = $q->fetchRow()) {
		$clientData = $row;
		if (
			($row["cli_status"] == 'Pending_New_Client_Sales' && $_SESSION["auth"]["default_scope"] == 'Sales')
			||
			($row["cli_status"] == 'Pending_New_Client_Lettings' && $_SESSION["auth"]["default_scope"] == 'Lettings')
		) {

			$alert = '<p>This client is in the pending list - <a href="new_client_questionaire.php?cli_id=' . $row["cli_id"] . '">click here to complete the new client questionaire</a></p>';
		}
...
if ($clientData['addressID']) {
	$sql            = "SELECT * FROM address WHERE id = '" . $clientData['addressID'] . "' LIMIT 1";
	$primaryAddress = $db->query($sql)->fetchRow();
}

if ($clientData['secondAddressID']) {
	$sql           = "SELECT * FROM address WHERE id = '" . $clientData['secondAddressID'] . "' LIMIT 1";
	$secondAddress = $db->query($sql)->fetchRow();
}
//echo "<pre style='color:blue' title='" . __FILE__ . "'>" . basename(__FILE__) . ":" . __LINE__ . "<br>";
//print_r($secondAddress);
//echo "</pre>";
...
$render_addresses = '<input type="button" id="address_searchButton" value="search">';
include_once dirname(__FILE__) . '/_clientAddressForm.php';
$render_addresses = '<label class="formLabel">Primary Address</label><div style="float:left; line-height:14px;">' . clientAddressForm('Address', $primaryAddress) . '</div> <div style="clear:both;">';
$render_addresses .= '<label class="formLabel">Correspondence Address</label><div style="float:left">' . clientAddressForm('SecondAddress', $secondAddress) . '</div> <div style="clear:both;">';
	...
	while ($row = $q->fetchRow()) {
		...
		if ($row["app_type"] == 'Viewing') {
			...
			if (strtotime($row["app_end"]) < strtotime($date_mysql)) {
				if (!$row["d2a_feedback"]) {
					$feedback = '<a href="/admin4/appointment/feedback/id/' . $row["d2a_id"] . '">(not entered)</a>';
				} else {
					$feedback = '<a href="/admin4/appointment/feedback/id/' . $row["d2a_id"] . '">' . $row["d2a_feedback"] . '</a>';
				}
			} else {
				$feedback = '(in future)';
			}
...
<table cellpadding="2" cellspacing="2" border="0">
  ...
  <tr>
    <td class="label">Registered/Activated by</td>
	<td>' . $regd_name . '</td>
  </tr>
  <tr>
...
/////////////////////////////////////////////////////////////////////////////////
	...
	$form->addHtml(renderNotes('client_general', $cli_id, array(
															   'viewform' => '1',
															   'label'    => 'General Notes'
														  )));
...
// show old address from import
	if ($cli_oldaddr && !$render_addresses) {
		...
		$form->addHtml($form->addRow('textarea', 'cli_oldaddr', 'Old Address', $cli_oldaddr, array(
																								  'style'    => 'width:400px',
																								  'readonly' => 'readonly'
																							 ), '', ''));
...
//															 ));
	...
	if ($cli_sales !== 'Yes') {
		...
		$form->addHtml($form->addDiv($form->makeField("button", $formName, "", "Register", array(
																								'class'   => 'submit',
																								'onClick' => 'javascript:swapDiv(\'sale\',\'sale2\');'
																						   ))));
		$form->addHtml('</div>');
	}
...
/////////////////////////////////////////////////////////////////////////////////
	...
	if ($cli_lettings !== 'Yes') {
		...
		$form->addHtml($form->addDiv($form->makeField("button", $formName, "", "Register", array(
																								'class'   => 'submit',
																								'onClick' => 'javascript:swapDiv(\'let\',\'let2\');'
																						   ))));
		$form->addHtml('</div>');
	}
...
// only show if client is registered for sales or lettings
	if ($cli_sales == "Yes" || $cli_lettings == "Yes") {
		...
		$form->addData($$formName, $_POST);
		$form->addHtml(renderNotes('client_req', $cli_id, array(
															   'viewform' => 7,
...
function getSelectionId(text, li) {
	...
	<?php
	$page->addBodyContent(ob_get_clean());
	if ($msg) {
...
// validate the appropriate data array+form combintation, except for form2 (addresses) which is dealt with separately
	...
	if ($viewForm !== 2 && $viewForm !== 6 && $viewForm !== 7) {
...
// build return link
	...
	if (isset($_POST['Address']['id']) && $_POST['Address']['id']) {
		$db_data['addressID'] = $_POST['Address']['id'];
	}
	if (isset($_POST['SecondAddress']['id']) && $_POST['SecondAddress']['id']) {
		$db_data['secondAddressID'] = $_POST['SecondAddress']['id'];
	}
