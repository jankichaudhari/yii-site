...
if (isset($_GET['cli_name'])) {
	$_GET['cli_name'] = mysql_real_escape_string($_GET['cli_name']);
}
...
// build sql
		if (trim($_GET["cli_email"])) {
			$sql_email = " cli_email LIKE '%" . trim(addslashes($_GET["cli_email"])) . "%' OR ";
		}
...
// no errors, continue
			...
			if (!$_POST["noaddress"]) {
				...
				if ($_POST["pro_pro_id"]) {
					...
					admin_notify(array('subject'=> 'Manual Property Entry',
