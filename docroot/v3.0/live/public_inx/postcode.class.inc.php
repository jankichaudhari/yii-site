...
function lookup($_type,$_address,$_style="select") {
	...
	if(count($Data)==0){
		...
		$Data = $this->render($_url);
		*/
		$errors[] = "No matching addresses were found, please check your details and try again. Try searching just 
		street name and first part of the postcode to see a list of all properties in that street.<br>
		<br>If the problem persists, the address you are searching for may not be present in the Royal Mail 
		database, in which case you may enter the address <a href=\"javascript:ajax_manual();\">manually</a>.";
		//echo error_message($errors,'javascript:cancelResponse();');
		//exit;
		}
...
function output_list($Data) {
	$output = '<select name="udprn" id="udprn" size="4" style="width:400px">'."\n";	 //document.forms[0].submit();
	foreach ($Data as $keyd => $data) {
