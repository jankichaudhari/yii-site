<?php
// formatting and validation functions (number, price, string etc)
...
function check_email($email) {
	...
	if (!eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) { 
		...
		// removed as some valid email address will fail (e.g. mark@markdw.com give "connection refused")
		/*if(!fsockopen($domain,25,$errno,$errstr,1)) {
			return false;
			}*/
		return true;
		}
...
function format_description($_str=null) {
	$_str = strip_tags($_str,'<p></p><li></li><a></a>');
	$_str = preg_replace("/[\r\n]+[\s\t]*[\r\n]+/","",$_str);
...
function numbers_only($_input=NULL) {
	...
	$_output = (float) preg_replace('/[^0-9.]*/','',$_input); 
	return $_output;
	unset($_input,$_output);
	}
// convert weekly price to monthly
function pw2pcm($_input="0") {
	$_output = round(($_input * 52) / 12);
	return $_output; 
	unset($_input,$_output);	
}
