	...
	*/
	...
	foreach($_fields AS $field_name => $field_array) {		
		...
		foreach($field_array AS $title => $value) {
			...
			elseif ($title == 'options' && is_array($value)) {
				$options = $value;
				}
			else {
				$title = trim($title);
	...
	function makeField($_type,$_name,$_label=NULL,$_value=NULL,$_attr=NULL,$_opt=NULL) {		
		...
		elseif ($_type == "select_number") {
			...
			if (!$_opt['min']) { $_opt['min'] = '0'; }
			if (!$_opt['max']) { $_opt['max'] = '10'; }
	...
	function addForm($_name="form",$_method="get",$_action=NULL,$_enctype="text",$_js=NULL) {
	...
	function addLabel($_name,$_label,$_input,$_href=NULL) {	
		...
		$_output = '<label for="'.$_name.'x" class="formLabel" id="label">'.$_label_ref.'</label>'."\n".$_input;		
	...
	function makeTooltip($_tooltip) {
		...
		return '<div id="tooltip"><a href="javascript:void(0);" onMouseover="showhint(\''.$_tooltip.'\', this, event, \'180px\')"><img src="'.GLOBAL_URL.'/img/icons/help.gif" width="16" height="16" border="0"></a></div>';
		}
	...
	function ajaxPostcode($_lookup_type="by_freetext",$_scope="pro") {	
		...
		if ($_lookup_type == "by_freetext") {
			...
			$_output .= $this->addRow("text","street","Street","",array('style'=>'width:320px;'));
			$ajax_fields = $this->makeField("text","postcode","","",array('style'=>'width:100px;'));
			$ajax_fields .= $this->makeField("button","","","Get Address",array('class'=>'button','onClick'=>'javascript:ajax_lookup();'));
			$ajax_fields .= '<div id="systemWorking" style="display:none" class="inline"><img src="'.GLOBAL_URL.'img/ajax-loader.gif" width="16" height="16" alt="Loading..." title="" /></div>';
		...
		$_output .= $this->addHtml('<div id="inset">If the address is outside the UK click <a href="javascript:ajax_manual();">here</a></div>');
...
class Validate {
	...
	function process($_fields,$_querystring=NULL) {
		...
		foreach($_fields AS $field_name => $field_array) {
			...
			if ($_fields[$field_name]['init']) {
				if ($_querystring[$field_name] == $_fields[$field_name]['init']) {
					if ($_fields[$field_name]['required'] == 2) {
			...
			if ($_fields[$field_name]['required'] == 2) { // && !$_querystring[$field_name] || !$_querystring[$field_name] === "0"
				...
				} elseif (!$_querystring[$field_name]) {
					$errors[$field_name] = $_fields[$field_name]['label']." is required";
					}
				}
			//else {
		...
		return array('Errors'=>$errors,'Results'=>$db_data);
		}
