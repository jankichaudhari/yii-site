	...
	var methods = {
		...
		init: function(options, node) {
			...
			return $(node).each(function(i) {
				...
				sliderSettings[sliderNumber] = settings;
				...
				var scrollerWidth;
				var clickEvent = isTouch ? 'touchstart.sliderEvent' : 'click.sliderEvent';
				var anchorEvents;
				var onclickEvents;
				...
				function init() {
					...
					if(isFirstInit) {
						settings.startAtSlide = (sliderSettings[sliderNumber].startAtSlide > childrenOffsets.length) ? childrenOffsets.length : sliderSettings[sliderNumber].startAtSlide;
						if(settings.infiniteSlider) {
							settings.startAtSlide = (sliderSettings[sliderNumber].startAtSlide - 1 + numberOfSlides)%numberOfSlides;
							activeChildOffsets[sliderNumber] = (sliderSettings[sliderNumber].startAtSlide);
						} else {
							settings.startAtSlide = ((sliderSettings[sliderNumber].startAtSlide - 1) < 0) ? childrenOffsets.length-1 : sliderSettings[sliderNumber].startAtSlide;
				...
				if(sliderSettings[sliderNumber].responsiveSlides || sliderSettings[sliderNumber].responsiveSlideContainer) {
					...
					$(window).bind(orientationEvent + '.sliderEvent', function() {
				...
				if(isTouch || settings.desktopClickDrag) {
					...
					var touchStartEvent = isTouch ? 'touchstart.sliderEvent' : 'mousedown.sliderEvent';
					...
					var touchMoveEvent = isTouch ? 'touchmove.sliderEvent' : 'mousemove.sliderEvent';
