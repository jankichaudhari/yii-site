...
if ($_GET["do"] == "remove_vendor") {
	// remove vendor from deal, delete row from cli2dea table
	$sql = "DELETE FROM cli2dea WHERE
	cli2dea.c2d_cli = ".$_GET["cli_id"]." AND
	cli2dea.c2d_dea = ".$_GET["dea_id"]	;
	$q = $db->query($sql);
...
foreach($vendors AS $ven_id => $ven_name) {
	...
	if (count($vendors) == 1) {
		...
		$vendor_table .= '<a href="?do=remove_vendor&dea_id='.$dea_id.'&cli_id='.$ven_id.'&return='.urlencode('?'.$_SERVER['QUERY_STRING']).'"><img src="'.GLOBAL_URL.'/img/icons/cross_sm.gif" border="0" alt="Remove '.$ven_name.' from this Deal"></a>';
		}
	$vendor_table .= '</td></tr>';
	}
...
foreach($tenants AS $ven_id => $ven_name) {
	...
	//if (count($tenants) == 1) {
		//$tenant_table .= '';
		//} else {
		$tenant_table .= '<a href="?do=remove_vendor&dea_id='.$dea_id.'&cli_id='.$ven_id.'&return='.urlencode('?'.$_SERVER['QUERY_STRING']).'"><img src="'.GLOBAL_URL.'/img/icons/cross_sm.gif" border="0" alt="Remove '.$ven_name.' from this Deal"></a>';
		//}
	$tenant_table .= '</td></tr>';
	}
...
while ($row = $q->fetchRow()) {
	...
	if ($_SESSION["auth"]["default_scope"] == 'Lettings' && $dea_type == 'Lettings') {
		...
		$edit_link = '<a href="offer_edit.php?off_id='.$row["off_id"].'&searchLink='.$_SERVER['SCRIPT_NAME'].'?'.urlencode(replaceQueryString($_SERVER['QUERY_STRING'],'viewForm').'&viewForm=7').'"><img src="'.GLOBAL_URL.'/img/icons/page_edit.gif" width="16" height="16" border="0" alt="View/Edit this offer"/></a>';
...
if ($numRows) {
	...
	if ($row["app_type"] == 'Viewing') {
		...
		if ($row["use_colour"]) {
			$use_colour = '<span class="use_col" style="background-color: #'.$row["use_colour"].';"><img src="'.GLOBAL_URL.'img/blank.gif" width="10" height="10"></span>&nbsp;';
			} else { // unnassigned
			$use_colour = '<span class="use_col" style="background-color: #FFFFFF;"><img src="'.GLOBAL_URL.'img/blank.gif" width="10" height="10"></span>&nbsp;';;
			}
		if (!$row["use_name"]) {
  ...
  <tr>
	...
	<td width="5%" align="right" valign="top">
	  <a href="appointment_edit.php?app_id='.$row["app_id"].'&searchLink='.$_SERVER['SCRIPT_NAME'].urlencode('?'.replaceQueryString($_SERVER['QUERY_STRING'],'viewForm').'&viewForm=5').'"><img src="'.GLOBAL_URL.'/img/icons/page_edit.gif" width="16" height="16" border="0" alt="View/Edit Appointment"/></a>
	  </td>
  </tr>';
			$countViewing++;
...
$formData3 = array(
	...
	'dea_board_log'=>array(
		...
		'label'=>'History',
		'value'=>'History',
		'attributes'=>array('class'=>'button','onClick'=>'window.open(\'changeLog.php?scope=board&dea_id='.$dea_id.'\',\'logWin\',\'width=550,height=400,scrollbars=1\');'),
		'group'=>'Board',
		'last_in_group'=>1
		),
	'dea_branch'=>array(
...
$buttons = $form->makeField("button",$formName,"","Submit Offer",array('onClick'=>'javascript:document.location.href=\'client_lookup.php?dest=offer&dea_id='.$dea_id.'&return='.urlencode($_SERVER['QUERY_STRING']).'\';','class'=>'submit'));
