...
global $protected;
/*
secured version of property edit page.
permissions per page (administrator has access to everything):

Details (top)		Administrator,Editor, Production, Negotiator, Manager
Details (bottom)	Administrator,Editor, Production
Descriptions		Administrator,Editor, Production (when live, only editor)
Images				Administrator,Editor, Production
Floorplans			Administrator,Editor, Production
Map					Administrator,Editor, Production
Notes				Administrator,Editor, Production, Negotiator, Manager
Log					Administrator,Editor, Production

actions
proof 				Administrator,Editor,Production, Negotiator, Manager
activate			Administrator,Editor
archive				Administrator,Editor,Production, Negotiator, Manager

*/

// checks current user's loa against allowed array of loa's
//
function checkAccess($loa,$access) {

	if (!is_array($access)) {
		echo "checkAccess error, second argument must be an array";
		exit;
		}

	if (!in_array($loa,$access)) {
		echo "<p>You do not have sufficient permissions to perform this action</p>";
		echo "<p>Your account type is:</p><ul><li>".$_SESSION["s_loa"]."</li></ul>";
		echo "<p>To perform this action you must contact someone with the following account type:</p><ul>";
...
// disable element only works when property is not in pending
function disableElement($loa,$access,$type="text",$input=NULL) {
	...
	if (!in_array($loa,$access)) {
		if ($type == 'text') {
			...
			} elseif ($type == 'select' || $type == 'radio' || $type == 'submit') {
			$output = ' disabled="disabled"';
			} elseif ($type == 'textarea') {
			$output = 'disabled';
			}
		}
	// the protected variable is a global, and is set to 1 when the property is visible on site
	if ($GLOBALS["protected"] == 1) {
		return $output;
		}
	}

$pageTitle = "Property";
#echo "LOA = ".$_SESSION["s_loa"];
require("global.php");
require("secure.php");
...
if ($_GET["action"] == "undo") {
	checkAccess($_SESSION["s_loa"],array('Administrator','Editor','Production'));
	if (!$_GET["propID"]) {
...
elseif ($_GET["action"] == "proof") {
	...
	checkAccess($_SESSION["s_loa"],array('Administrator','Editor','Production','Negotiator','Manager'));
	...
	while ($row = $q->fetchRow()) {
		...
		$proofer_email = "mark@woosterstock.co.uk,gemma@woosterstock.co.uk";
...
elseif ($_GET["action"] == "activate") {
	...
	#checkAccess($_SESSION["s_loa"],array('Administrator','Editor'));
...
elseif ($_POST["action"] == "Update") {
	...
	if ($_POST["SubmitNote"]) {
		...
		if ($_POST["not_id"]) {
			...
			*/
			header("Location:?propID=".$intPropID."&view=notes&searchLink=".$_POST["searchLink"]."&changes=Update+Successful");
			exit;
			...
			(not_date, not_user, not_table, not_row, not_subject2, not_note, not_flag)
			VALUES
			('$dateToday', '".$_SESSION["s_userid"]."', 'property', '".$intPropID."', '".$_POST["not_subject2"]."', '".trim($_POST["not_note"])."', '".$_POST["not_flag"]."')";
...
foreach ($files as $key => $value) {
	...
	if (!$_POST["board"]) {
		...
		$fieldnames[] = "board";
		$newvalues[] = trim($_POST["board"]);
		}
	if (!$_POST["date_available"]) {


		} else {
		 if ($_POST["delete_date_available"] == "Yes") {
		 	$date_available = 'NULL';
		 	} else {
			$parts = explode("/",$_POST["date_available"]);
			$date_available = $parts[2]."-".$parts[1]."-".$parts[0]." 00:00:00";
			}

		$fieldnames[] = "date_available";
		$newvalues[] = date('Y-m-d H:i:s',strtotime($date_available));
		}
...
elseif ($_POST["action"] == "Insert") {
	...
	if (!$_POST["longDescription"]) {
		// room
		if ($type_id == 11) {
			$longDescription = '<p> [opening paragraph] </p>
<p><strong>Whats on offer:</strong><br/>
 [blurb] </p>
<p><strong>Benefits:</strong><br/>
 [blurb] </p>
<p><strong>Bills include:</strong></p>
<li> [bills (capitalised) </li>
<p><strong>This share would suit:</strong><br/>
 [blurb] </p>
<p><strong>Out and about:</strong><br/>
 [blurb] </p>';
	...
	if ($_POST["date_available"]) {
		$parts = explode("/",$_POST["date_available"]);
		$date_available = date('Y-m-d H:i:s',strtotime($parts[2]."-".$parts[1]."-".$parts[0]." 00:00:00"));
		}
	...
	total_area,image0,image1,image2,image3,image4,image5,image6,image7,image8,
	image9,image10,image11,image12,image13,image14,image15,banner,BannerLink,ThumbNail,
	Countdown,Dates,Hits,Neg,Branch,SaleLet,managed,furnished,lease_length,service_charge,ground_rent,other_details,card, suffix,date_available)
	VALUES
	('$Price','$PriceType','$Price2','$PriceType2','$house_number','$Address1','$Postcode','$osx','$osy','$lease_free','$state_of_trade_id','$type_id',
	'$area_id','$longDescription','$description','$gch','$doubleGlazed','$receptions','$bedrooms','$bathrooms','$garden','$parking',
	'$total_area','$image0','$image1','$image2','$image3','$image4','$image5','$image6','$image7','$image8',
	'$image9','$image10','$image11','$image12','$image13','$image14','$image15','$banner','$BannerLink','$ThumbNail',
	'$Countdown','$dateToday','$Hits','$Neg','$Branch','$SaleLet','$managed','$furnished','$lease_length','$service_charge','$ground_rent','$other_details','$card','$suffix','$date_available')";
	$q = $db->query($sql);
	//echo $sql;
...
foreach ($files as $key => $value) {
	...
	$EmailHeaders = "Content-Type:text/plain;CHARSET=iso-8859-8-1\r\n";
	$EmailHeaders	.="From:".$_SESSION["s_email"]."\r\n";
...
elseif (!$_POST["action"]) { // if form is not submitted
	...
	if (!$_GET["propID"]) { // if id not entered, show insert form
		...
		$sql = "SELECT *,DATE_FORMAT(date_available,'%d/%m/%Y') as date_available FROM property WHERE property.prop_ID = $intPropID LIMIT 1";
		$q = $db->query($sql);
		if (DB::isError($q)) {  die("property select error: ".$q->getMessage()); }
		...
		while ($row = $q->fetchRow()) {
			...
			if ($row["date_available"]) {
				if (strtotime($row["date_available"]) > strtotime('1970-01-01 00:59:59')) {
	...
	// protect the when it is not set to pending
	if ($intStatus == 11) {
		$GLOBALS["protected"] = 0;
		} else {
		$GLOBALS["protected"] = 1;
	...
	$strRenderNeg = '<option value="">(unassigned)</option>'.$strRenderNeg;
	...
	if (($strBranchTitle == 'Camberwell') && ($SaleLet == 2)) {
		$strBranchTel = '08456 800 460';
		}
	elseif (($strBranchTitle == 'Sydenham') && ($SaleLet == 2)) {
		$strBranchTel = '08456 800 464';
		}
  ...
  <table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    ...
    <tr>
      ...
      <td colspan="3" class="greyForm">
        ...
        Pending &nbsp; <a href="?action=<?php if (in_array($s_userid,$proofers)) { ?>proof<?php } else { ?>proof<?php } ?>&propID=<?php echo $intPropID; ?>&searchLink=<?php echo urlencode($searchLink); ?>"><strong>ACTIVATE</strong></a>
    ...
    <tr>
      ...
      <td class="greyForm"><input type="text" name="house_number" value="<?php echo $strNumber; ?>" style="width: 190px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>>
      </td>
	  <td align="right" class="greyForm">Property Type</td>
      <td width="200" colspan="3" class="greyForm"><select name="type_id" style="width: 190px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php echo $strRenderPropType; ?> </select></td>

    </tr>
    <tr>
      <td class="greyForm" align="right">Street</td>
      <td class="greyForm"><input type="text" name="Address1" value="<?php echo $strStreet; ?>" style="width: 190px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>>
      </td>
      <td align="right" class="greyForm">C/Heating</td>
      <td class="greyForm"><input type="checkbox" name="gch" value="1"<?php if ($strGCH) { echo " checked"; } ?><?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'radio'); ?>>
      </td>
      <td align="right" class="greyForm">D/Glazing</td>
      <td class="greyForm"><input type="checkbox" name="doubleGlazed" value="1"<?php if ($strDG) { echo " checked"; } ?><?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'radio'); ?>></td>
    </tr>
    <tr>
      <td class="greyForm" align="right">Area</td>
      <td class="greyForm"><select name="area_id" style="width: 190px;" onChange="populateCoords()"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php echo $strRenderArea; ?> </select> </td>
      <td align="right" class="greyForm">Tenure</td>
      <td colspan="3" class="greyForm"><select name="lease_free" style="width: 190px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php echo $strRenderTenure; ?> </select> <input type="hidden" name="furnished" value="0"></td>
    </tr>
    <tr>
      <td class="greyForm" align="right">Postcode</td>
      <td class="greyForm"><input name="Postcode" type="text" value="<?php echo $strPostcode; ?>" size="8" maxlength="8" style="width: 80px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>>
      </td>
    ...
    <tr>
      <td class="greyForm" align="right">Qualifier </td>
      <td class="greyForm"> <select name="suffix" style="width: 100px;">
          <?php echo db_enum("property","suffix",$intSuffix); ?>
        </select> <!--<input disabled="disabled" name="Price2" type="text" style="width: 80px;" onKeypress="if (event.keyCode < 48 || event.keyCode > 58) event.returnValue = false;" value="<?php echo $intPrice2; ?>" size="8" maxlength="8">
        <select name="PriceType2" disabled="disabled">
    ...
    <tr>
      ...
      <td align="right" class="greyForm">C/Heating</td>
      <td width="200" class="greyForm"><input type="checkbox" name="gch" value="1"<?php if ($strGCH) { echo " checked"; } ?><?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'radio'); ?>>
      </td>
      <td align="right" class="greyForm">D/Glazing</td>
      <td width="200" class="greyForm"><input type="checkbox" name="doubleGlazed" value="1"<?php if ($strDG) { echo " checked"; } ?><?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'radio'); ?>></td>
    </tr>
    <tr>
      <td class="greyForm" align="right">Area</td>
      <td class="greyForm"><select name="area_id" style="width: 190px;" onChange="populateCoords()"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
    ...
    <tr>
      ...
      <td align="right" class="greyForm">Available from </td>
      <td colspan="3" class="greyForm"><input type="text" name="date_available" value="<?php echo $date_available; ?>" style="width:90px" onClick="popUpCalendar(this, form.date_available, 'dd/mm/yyyy')">
      Delete?
      <input name="delete_date_available" type="checkbox" id="delete_date_available" value="Yes"></td>
    </tr>
    <?php } ?>
    <tr align="center">
      <td colspan="6" class="greyForm">Receptions:
        <select name="receptions"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php
		for ($i = 1; $i <= 9; $i++) {
			echo '<option value="'.$i.'"';
			if ($i == $intReceptions) {
        ...
        <select name="bedrooms"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php
		for ($i = 0; $i <= 9; $i++) {
			echo '<option value="'.$i.'"';
			if ($i == $intBedrooms) {
        ...
        <select name="bathrooms"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php
		for ($i = 1; $i <= 9; $i++) {
			echo '<option value="'.$i.'"';
			if ($i == $intBathrooms) {
    ...
    <tr>
      ...
      <td class="greyForm"><select name="garden" style="width: 190px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php echo db_enum("property","garden",$intGarden); ?> </select></td>
      <td align="right" class="greyForm">Parking</td>
      <td colspan="3" class="greyForm"><select name="parking" style="width: 190px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
          <?php echo db_enum("property","parking",$intParking); ?> </select></td>
    </tr>
    <tr>
      <td class="greyForm" align="right">Board</td>
      <td class="greyForm"><select name="board" style="width: 190px;"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'select'); ?>>
  ...
  <table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    ...
    <tr>
      ...
      <td><em><?php echo $strTenureTitle; ?> with some
          <input name="lease_length" type="text" id="lease_length" style="font-style:italic" size="4" maxlength="4" value="<?php echo $strLeaseLength; ?>"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>>
years remaining on the lease</em></td>
    </tr>
    <tr>
      <td nowrap><strong>Service Charge:</strong> </td>
      <td><input name="service_charge" type="text" id="service_charge" style="width:450px; font-style:italic" value="<?php echo $strServiceCharge; ?>" maxlength="220"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
      <td><strong>Grount Rent:</strong> </td>
      <td><input name="ground_rent" type="text" id="ground_rent" style="width:450px; font-style:italic" value="<?php echo $strGroundRent; ?>" maxlength="220"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
      <td><strong>Other Details: </strong></td>
      <td><input name="other_details" type="text" id="other_details" style="width:450px; font-style:italic" value="<?php echo $strOtherDetails; ?>" maxlength="220"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
  </table>
	<?php } ?>
	<table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
...
<table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    ...
    <tr>
      <td class="greyForm"><input name="description" type="text" style="width:580px;" value="<?php echo $strDescription; ?>" maxlength="110"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor')); ?>></td>
    </tr>
    <tr>
    ...
    <tr>
	  ...
	  #echo "<p>".disableElement($_SESSION["s_loa"],array('Administrator','Editor'),'textarea')."</p>";

	 // if (disableElement($_SESSION["s_loa"],array('Administrator','Editor'),'textarea') == 'disabled') {
	 // echo '<div style="background-color: white; ">'.$strLongDescription.'</div>';
	  //}
	  //else {
	  ?>
  ...
  <table align="center" width="410" border="0" cellspacing="5" cellpadding="5">
    <tr>
	  ...
	  ?><a href="javascript:confirmDelete('Delete Image?','?action=delete_image&propID=<?php echo $intPropID; ?>&view=<?php echo $view; ?>&image=0&file=<?php echo $strImage0; ?>&searchLink=<?php echo urlencode($searchLink); ?>');"><img src="<?php echo $image_folder.$strImage0; ?>" alt="Click to delete this image" width="400" height="400" border="0"></a><br>
	    <?php } ?>
	    Main Image
      <input type="file" name="image[0]"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
	  ...
	  ?><a href="javascript:confirmDelete('Delete Image?','?action=delete_image&propID=<?php echo $intPropID; ?>&image=1&view=<?php echo $view; ?>&file=<?php echo $strImage1; ?>&searchLink=<?php echo urlencode($searchLink); ?>');"><img src="<?php echo $image_folder.$strImage1; ?>" alt="Click to delete this image" width="200" height="200" border="0"></a><br>
	    <?php }  ?>
      <input type="file" name="image[1]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
	  ...
	  ?><a href="javascript:confirmDelete('Delete Image?','?action=delete_image&propID=<?php echo $intPropID; ?>&image=2&view=<?php echo $view; ?>&file=<?php echo $strImage1; ?>&searchLink=<?php echo urlencode($searchLink); ?>');"><img src="<?php echo $image_folder.$strImage2; ?>" alt="Click to delete this image" width="200" height="200" border="0"></a><br>
	    <?php } ?>
      <input type="file" name="image[2]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
	  ...
	  ?><a href="javascript:confirmDelete('Delete Image?','?action=delete_image&propID=<?php echo $intPropID; ?>&image=3&view=<?php echo $view; ?>&file=<?php echo $strImage3; ?>&searchLink=<?php echo urlencode($searchLink); ?>');"><img src="<?php echo $image_folder.$strImage3; ?>" alt="Click to delete this image" width="200" height="200" border="0"></a><br>
	    <?php } ?><input type="file" name="image[3]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>>
      </td>
      ...
      <?php } ?><input type="file" name="image[4]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
      ...
      <?php } ?><input type="file" name="image[5]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
      ...
      <?php } ?><input type="file" name="image[6]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
      ...
      <?php } ?><input type="file" name="image[7]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
      ...
      <?php } ?><input type="file" name="image[8]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
      ...
      <?php }  ?><input type="file" name="image[9]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
      ...
      <?php } ?><input type="file" name="image[10]" style="width:200px"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
  </table>
  <table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    <tr>
  ...
  <table align="center" border="0" cellspacing="5" cellpadding="5">
    <tr>
      ...
      <td class="greyForm"><input name="total_area" type="text" value="<?php echo $strTotalArea; ?>" size="4" maxlength="6"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>>
      square meters</td>
    </tr>
    <tr>
	  ...
	  Floorplan 1  <input type="file" name="image[11]"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr>
	...
	if ($strImage12) {
		...
		} ?>Floorplan2 <input type="file" name="image[12]"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr align="center">
	...
	if ($strImage13) {
	  ...
	  Floorplan3 <input type="file" name="image[13]"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr align="center">
      <td colspan="2" class="greyForm">
	<?php
	if ($strImage14) {
	  ...
	  Floorplan4  <input type="file" name="image[14]"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
    <tr align="center">
      <td colspan="2" class="greyForm">
	<?php
	if ($strImage15) {
		...
		Floorplan5 <input type="file" name="image[15]"<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production')); ?>></td>
    </tr>
  </table>
  <table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    <tr>
  ...
  <table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    <tr>
	  ...
	  <?php if ($_GET["y"]) { ?>
      <input type="hidden" name="osx" value="<?php echo $osx; ?>">(<?php echo $osx; ?> x
	  <input type="hidden" name="osy" value="<?php echo $osy; ?>"><?php echo $osy; ?>)
	  <?php } ?></th>
    </tr>
  </table>
  <table align="center" width="600" border="0" cellspacing="1" cellpadding="0">
    ...
    <tr>
      <td colspan="3" align="center"><hr noshade size="1"> <input name="Submit" type="submit" value="   <?php echo $action; ?> Property   "<?php echo disableElement($_SESSION["s_loa"],array('Administrator','Editor','Production'),'submit'); ?>>
        <hr noshade size="1"> </td>
    </tr>
    <tr><td colspan="3"></tr>
  </table>
	...
	<tr>
      ...
      <td class="greyForm"><select name="not_subject2">
       '.db_enum("note","not_subject2",$not_subject2).'
	   </select></td>
	</tr>
	<tr>
...
WHERE not_table = 'property' AND not_row = '$intPropID'
$extra_sql
ORDER BY ".$_GET["order"];
...
while ($row = $qNotes->fetchRow()) {
	$notesTable .= '
	<tr>
	<td><strong>'.$row["not_subject2"].'</strong> by <strong>'.$row["adm_name"].'</strong> on '.$row["date"].'</td>
	<td>Flag: <span class="flag'.$row["not_flag"].'"><img src="images/flag'.$row["not_flag"].'.gif" align="absmiddle">'.$row["not_flag"].'</span></td>
	<td>[ <a href="?propID='.$intPropID.'&view=notes&amp;not_id='.$row["not_id"].'">Edit</a> ]</td>
	</tr>
	<tr>
...
$link = 'propID='.$intPropID.'&amp;view='.$_GET["view"];
?>
  ...
  <a name="newnote"></a>
	...
	<tr>
	  <td width="20%" align="right" class="greyForm">Type</td>
      <td class="greyForm"><select name="not_subject2">
       <?php echo db_enum("note","not_subject2"); ?>
      </select></td>
	</tr>
	<tr>
	  <td align="right" class="greyForm">Note</td>
 ...
 <table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    ...
    <tr>
      <td><span class="greyForm">
        <textarea readonly="true" name="notes" cols="30" rows="10" style="width: 590px;"><?php echo $strNotes; ?></textarea>
      </span></td>
    </tr>
</table>
	<?php } ?>
...
<table align="center" width="600" border="0" cellspacing="3" cellpadding="4">
    ...
    <tr>
      <td><a href="mailto:?subject=Wooster%20%26%20Stock&body=Dear CLIENT,%0D%0A%0D%0A<?php echo str_replace('&','&amp;',$strDescription); ?>%0D%0A<?php echo $strStreet; ?>, <?php echo str_replace("&","and",$strArea); ?> <?php echo $strPC1; ?>%0D%0A<?php echo $strPrice; ?> <?php if ($SaleLet == 1) { echo $strTenureTitle; } else { echo "p/w"; } ?>%0D%0Ahttp://www.woosterstock.co.uk/Detail.php?propID=<?php echo $intPropID; ?>%0D%0A%0D%0ATo arrange a viewing or for more details, please contact our <?php echo $strBranchTitle; ?> office on <?php echo $strBranchTel; ?>%0D%0A%0D%0ARegards,%0D%0A%0D%0A<?php echo $_SESSION["s_name"];?>%0D%0AWooster %26 Stock">Email this property to a Client</a></td>
    </tr>
	<?php } else { ?>
	<tr>
