...
$sql = "SELECT *, date_format(clients.DateCreated,'%d/%m/%Y') as date FROM clients WHERE ";
...
if ($_GET["PropType"] && $_GET["PropType"] <> "Any") {
	$sql .= " (clients.PropertyTypeLet = '".$_GET["PropType"]."' OR clients.PropertyTypeLet = 'Any') AND ";
...
if ($_GET["Price"]) {
$sql .= " clients.MinPriceLet <= ".$_GET["Price"]." AND clients.MaxPriceLet >=".$_GET["Price"]." AND ";
...
$pages = '            
	  <tr> 
		<td class="podTextCenter" colspan="2"><b>Found 0 clients</b></td>
	  </tr>
	  ';
...
$render = '		
        <table border="0" cellspacing="2" cellpadding="2" width="700">
		  <tr>
		    <td><a href="client_search_lettings.php">New Search</a> - '.$data['numrows'].' records found</td>
...
while ($row = $pager->fetchRow(DB_FETCHMODE_ASSOC)){
	if ($row["MinPriceLet"] == 0) {
		$MinPrice = "No Min";
		}
	else {
		$MinPrice = price_format($row["MinPriceLet"]);
		}
	if ($row["MaxPriceLet"] == 999999999) {
		$MaxPrice = "No Max";
		}
	else {
		$MaxPrice = price_format($row["MaxPriceLet"]);
		}
	...
	$render .= '
        <table border="0" cellspacing="1" cellpadding="1" width="700">
          <tr> 
            ...
            <td width="80">
			<a href="client.php?ClientID='.$row["Client_ID"].'&searchLink='.$PHP_SELF.'?'.urlencode($_SERVER['QUERY_STRING']).'">Edit</a> ::
			<a href="prop_search_lettings.php?action=Search&PriceFrom='.$row["MinPriceLet"].'&PriceTo='.$row["MaxPriceLet"].'&BedFrom='.$row["BedroomsLet"].'&PropType='.$row["PropertyType"].'&Status[]=1&Status[]=2&Order=state_of_trade_id,Price&searchLink='.$PHP_SELF.'?'.urlencode($_SERVER['QUERY_STRING']).'">Match</a>
			</td>
          </tr>
          <tr> 
...
<form name="form" method="GET">
    <table width="650" border="0" cellpadding="5" cellspacing="3">
      <tr> 
        <td class="pageTitle">Lettings Client Search</td>
        <td align="right"><a href="index.php">Main Menu</a></td>
      </tr>
    </table>
    <table width="650" border="0" cellpadding="4" cellspacing="2">
      ...
      <tr> 
        ...
        <td><input name="Keyword" type="text" value="" style="width:200px"></td>
      </tr>
      <tr> 
        ...
        <td class="greyText"><select name="Branch" style="width:200px">
            <option value="" selected>Any</option>
            <?php
      ...
      <tr> 
        ...
        Include Archived and Sales Clients </td>
        <td align="right" nowrap><span class="greyText">Negotiator</span></td>
        <td nowrap><select name="neg" id="neg" style="width:200px">
          <option value="" selected>Any</option>
          <?php
      ...
      <tr> 
        <td height="50" colspan="4" align="center"><input type="submit" value="Search Clients"> 
