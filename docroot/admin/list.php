...
//if ($_SESSION["s_loa"] <> "Administrator") {
	...
	while ($row = $q->fetchRow()) {
		if ($row["mai_date"] > $dateLast20Mins) {
			...
			/*$dbtime = strtotime($row["mai_date"]);
			$nowtime = strtotime(date('Y-m-d H:i:s'));
			echo '<!--';
			echo $dbtime."\n".$nowtime."\n";
			echo ($nowtime-$dbtime)."\n";
			echo round(($nowtime-$dbtime)/60)." mins to go before you can send this mailshot, please wait...<br>\n-->";*/
			echo 'Only one mailshot is allowed every 20 minutes. Last one sent '.$row["mai_date"].', time now '.date('Y-m-d H:i:s');
...
// workaround for new telephone numbers
if ($salelet == 2) {
	if ($branch_id == 1) {
		$tel = '08456 800 460';
		} elseif ($branch_id == 2) {
...
//echo "branch: $branch_id<br>salelet: $salelet<br>tel: $tel";
