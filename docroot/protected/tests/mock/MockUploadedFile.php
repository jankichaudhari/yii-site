 ...
 * @since   1.0
 */
class MockUploadedFile extends CUploadedFile
{
	...
	protected static function collectFilesRecursive($key, $names, $tmp_names, $types, $sizes, $errors)
	{
		...
		if (is_array($names)) {
			...
			static::$_files[$key] = new CUploadedFile($names, $tmp_names, $types, $sizes, $errors);
		}
	}
	...
	public function saveAs($file, $deleteTempFile = true)
	{
		...
		if ($this->_error == UPLOAD_ERR_OK) {
			return copy($this->_tempName, $file); // for test purposes we don't delete uplaoded file;
		} else {
			return false;
		}
	}
