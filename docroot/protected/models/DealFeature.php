 ...
 */
class DealFeature extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return DealFeature the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('fea_weight', 'required'),
			array('fea_weight', 'numerical', 'integerOnly' => true),
			array('fea_title', 'length', 'max' => 100),
			array('fea_type', 'length', 'max' => 8),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('fea_id, fea_title, fea_type, fea_weight', 'safe', 'on' => 'search'),
		);
	}
