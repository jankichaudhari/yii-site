...
$GLOBALS['WKPDF_BASE_PATH'] = __DIR__ . "/";
...
class WKPDF
{
	...
	private $resourceTypes = ['css', 'js', 'image'];
	...
	private $margins = array('top' => 16, 'right' => 16, 'bottom' => 23, 'left' => 16);

	public function setMargins($margins = array('top' => 16, 'right' => 16, 'bottom' => 23, 'left' => 16))
	{
	...
	private static function _getCPU()
	{
		...
		if (self::$cpu == '') {
			if (`grep -i amd /proc/cpuinfo` != '') {
				self::$cpu = 'amd64';
			} elseif (`grep -i intel /proc/cpuinfo` != '') self::$cpu = 'i386'; else throw new Exception('WKPDF couldn\'t determine CPU ("' . `grep -i vendor_id /proc/cpuinfo` . '").');
		}
		return self::$cpu;
	}
	...
	public function __construct()
	{
		...
		$this->cmd = Yii::app()->params['WKPDF']['path'];
		...
		do {
			...
			$this->tmpHeader = $GLOBALS['WKPDF_BASE_PATH'] . 'tmp/' . mt_rand() . '_header.html';
			$this->tmpFooter = $GLOBALS['WKPDF_BASE_PATH'] . 'tmp/' . mt_rand() . '_footer.html';
	...
	public function set_html($html)
	{
		...
		$this->html = $this->filterHtml($html);
	...
	public function render()
	{
		...
		$file      = $GLOBALS['WKPDF_BASE_PATH'] . 'tmp/' . basename($this->tmp);
		$command   = '"' . $this->cmd . '"'
				. ' --dpi 300'
				. (($this->copies > 1) ? ' --copies ' . $this->copies : '') // number of copies
				. ' --orientation ' . $this->orient // orientation
				. ' --page-size ' . $this->size // page size
				. ($this->htmlHeader ? ' --header-html ' . $this->tmpHeader : '')
				. ($this->htmlFooter ? ' --footer-html ' . $this->tmpFooter : '')
				. ($this->toc ? ' --toc' : '') // table of contents
				. ($this->grayscale ? ' --grayscale' : '') // grayscale
				. (($this->title != '') ? ' --title "' . $this->title . '"' : '') // title
				. ((isset($this->margins['top']) ? " --margin-top " . $this->margins['top'] : ''))
				. ((isset($this->margins['right']) ? " --margin-right " . $this->margins['right'] : ''))
				. ((isset($this->margins['bottom']) ? " --margin-bottom " . $this->margins['bottom'] : ''))
				. ((isset($this->margins['left']) ? " --margin-left " . $this->margins['left'] : ''))
				. ' "' . $file . '" -';
		...
		file_put_contents(Yii::app()->params['filePath'] . 'wkpdf.log', $command);
		unlink($this->tmp);
		if (file_exists($this->tmpHeader)) {
			unlink($this->tmpHeader);
		}
		if (file_exists($this->tmpFooter)) {
			unlink($this->tmpFooter);
		}
	 ...
	 * @param string $file Th7e PDF's filename (the usage depends on $mode.
	...
	public function set_htmlHeader($html)
	{

		$this->htmlHeader = $html;
		file_put_contents($this->tmpHeader, $html);
	}

	public function set_htmlFooter($html)
	{

		$this->htmlFooter = $html;
		file_put_contents($this->tmpFooter, $html);
	}

	/**
	 * @param $type String type of the resource may be 'js', 'css', 'image'
	 * @param $path String path to the file
	 *
	 * @return \WKPDF
	 */
	public function addResource($type, $path)
	{
		...
		if (!file_exists($path)) {
			throw new InvalidArgumentException('path: ' . $path . ' does not exist or is not avaialable');
		}

		$folderName = $GLOBALS['WKPDF_BASE_PATH'] . 'tmp/' . $type . '/';
		$this->createDir($folderName);

		$destination = $folderName . basename($path);
		if (!copy($path, $destination)) {
			throw new RuntimeException('cannot copy a file ' . $path . ' to ' . $destination . '');
		}
		return $this;
	}
	...
	/**
	 * @param $type String type of the resource may be 'js', 'css', 'image'
	 * @param $name String name of the resource; previously registered
	 * @return string returns relative path to the resource that is available to use in HTML
	 * @throws InvalidArgumentException
	 */
	public function getResource($type, $name)
	{

		$path = $GLOBALS['WKPDF_BASE_PATH'] . 'tmp/' . $type . '/' . $name;
		if (!file_exists($path)) {
			throw new InvalidArgumentException('resource ' . $name . ' of type ' . $type . ' could not be found');
		}
		return $type . '/' . $name;
	}

	/**
	 * Helper function to add image to the pdf. can be used in <img src='$obj->addImage($path)'>
	 *
	 * @param $path String full path to the image file in file system or URL
	 * @return string returns relative path to the resource that is available to use in HTML
	 */
	public function addImage($path)
	{

		$this->addResource('image', $path);
		return $this->getResource('image', basename($path));
	}

	public function deleteResources()
	{

		foreach ($this->resourceTypes as $value) {
			$files = glob($GLOBALS['WKPDF_BASE_PATH'] . 'tmp/' . $value . '/');
