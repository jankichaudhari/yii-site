...
class m121210_153334_merge_addresses extends CDbMigration
{

	public function up()
	{

		$this->createTable('suspiciosAddress', array(
													'id' => 'pk',
											   ));
		/**
		 * Select all addresses that repeat.
		 */
		$sql = "SELECT concat_ws(' ', line1, line2, line3,line4,line5,postcode) address, count(*) `count`, GROUP_CONCAT(id) ids, id from address group by concat(line1, line2, line3,line4,line5,postcode) HAVING `count` > 1 ORDER BY `count` DESC";
...
//			$params = ['addressesToMerge' => implode(',', $pieces)];
				...
				Yii::app()->db->createCommand("DELETE FROM address WHERE id in (" . rtrim(implode(',', $pieces), ' ,') . ")")->execute();
