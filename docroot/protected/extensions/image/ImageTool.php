...
class ImageTool {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(ImageTool::$allowed_types[$image_info[2]]))
		...
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => ImageTool::$allowed_types[$image_info[2]],
	 ...
	 * wish to use height as master dim, set $image->master_dim = ImageTool::HEIGHT
	 * This method is chainable.
	 *
	 * @throws  Kohana_Exception
	 * @param   integer  width
	 * @param   integer  height
	 * @param   integer  one of: ImageTool::NONE, ImageTool::AUTO, ImageTool::WIDTH, ImageTool::HEIGHT
	 * @return  object
	 */
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== ImageTool::NONE AND
				    $value !== ImageTool::AUTO AND
				    $value !== ImageTool::WIDTH AND
				    $value !== ImageTool::HEIGHT)
					return FALSE;
			break;
		}
