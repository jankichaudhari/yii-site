...
class Image_GD_Driver extends Image_Driver {
    ...
    public function flip($direction)
	{
		...
		if ($direction === ImageTool::HORIZONTAL)
		{
			for ($x = 0; $x < $width; $x++)
			{
		...
		elseif ($direction === ImageTool::VERTICAL)
		{
			for ($y = 0; $y < $height; $y++)
			{
	...
	public function resize($properties)
	{
		...
		if ($properties['master'] === ImageTool::AUTO)
		{
			// Change an automatic master dim to the correct type
			$properties['master'] = (($width / $properties['width']) > ($height / $properties['height'])) ? ImageTool::WIDTH : ImageTool::HEIGHT;
		}

		if (empty($properties['height']) OR $properties['master'] === ImageTool::WIDTH)
		{
			// Recalculate the height based on the width
			$properties['height'] = round($height * $properties['width'] / $width);
		}

		if (empty($properties['width']) OR $properties['master'] === ImageTool::HEIGHT)
		{
